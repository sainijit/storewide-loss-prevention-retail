...
from backend.api import camera_routes, poi_routes, search_routes
...
from backend.strategy.alert import LogAlertStrategy, WebSocketAlertStrategy
...
async def lifespan(app: FastAPI):
    ...
    if "websocket" in cfg.delivery_handlers:
    ...
    # ── Consumer ──
    consumer = EventConsumer(matching_service, event_service, alert_service, event_bus)

    # ── MQTT ──
    if cfg.mqtt_host:
        log.info("Starting MQTT consumer...")
        _mqtt_adapter = MQTTAdapter(on_event=consumer.handle_event)
        try:
            _mqtt_adapter.start()
        except Exception:
